...
namespace ImageProcessingProject
{
    public static class ImageProcessor
    {
        ...
        {
            Bitmap source = Get32BppImage(original); //gelen görüntüyü 32bit ARGB formatına çevirdim
            Bitmap result = new Bitmap(source.Width, source.Height, PixelFormat.Format32bppArgb); // daha sonra sonuç için aynı boyutta yeni bir görüntü oluşturdum
            BitmapData srcData = source.LockBits(new Rectangle(0, 0, source.Width, source.Height), ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
            BitmapData resData = result.LockBits(new Rectangle(0, 0, result.Width, result.Height), ImageLockMode.WriteOnly, PixelFormat.Format32bppArgb);
            unsafe
            {
                byte* src = (byte*)srcData.Scan0; //bellek adreslerini pointer olarak aldım, Scan0 ilk pikselin adresi
                byte* res = (byte*)resData.Scan0;
                int stride = srcData.Stride;
                for (int y = 0; y < source.Height; y++)
                {
                    ...
                    {
                        int idx = (y * stride) + (x * 4); //her pikselin bellekteki konumunu hesapladım
                        byte gray = (byte)(0.299 * src[idx + 2] + 0.587 * src[idx + 1] + 0.114 * src[idx]); //gri değeri hesapladım
                        res[idx] = gray; res[idx + 1] = gray; res[idx + 2] = gray; res[idx + 3] = 255; //hesapladığım gri değeri r, g, b nin üçüne de yazdım
                    }
                }
            }
            source.UnlockBits(srcData);
            ...
            unsafe
            {
                ...
                for (int y = 0; y < source.Height; y++)
                {
                    for (int x = 0; x < source.Width; x++)
                    {
                        ...
                        byte val = src[idx] > threshold ? (byte)255 : (byte)0; //gri değeri eşik değeriyle karşılaştırdım, büyükse 255 yani beyaz, değilse 0 yani siyah. geri kalanı ise gri dönüüşüm ile aynı
        ...
        public static Bitmap CropImage(Bitmap original, int x1, int y1, int x2, int y2)
        {
            ...
            x2 = Math.Min(source.Width, x2); y2 = Math.Min(source.Height, y2); //kullanıcının girdiği koordinatlar görüntü dışına çıkmasın diye sınır kontrolü yaptım
            int cropW = x2 - x1, cropH = y2 - y1;
            if (cropW <= 0 || cropH <= 0) return source;
            Bitmap result = new Bitmap(cropW, cropH, PixelFormat.Format32bppArgb); //kırpılacak alan kadar yeni bir bitmap oluşturdum
            BitmapData srcData = source.LockBits(new Rectangle(0, 0, source.Width, source.Height), ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
            BitmapData resData = result.LockBits(new Rectangle(0, 0, result.Width, result.Height), ImageLockMode.WriteOnly, PixelFormat.Format32bppArgb);
            unsafe
            {
                ...
                for (int y = 0; y < cropH; y++)
                {
                    for (int x = 0; x < cropW; x++)
                    {
                        ...
                        res[ri] = src[si]; res[ri + 1] = src[si + 1]; res[ri + 2] = src[si + 2]; res[ri + 3] = src[si + 3]; //kaynak görüntüdeki ilgili pikseli (x1, y1'den başlayacak) alıp yeni görüntüye yazdırdım, si kaynak indeksi, ri sonuç indeksi
                    }
                }
            }
            source.UnlockBits(srcData);
