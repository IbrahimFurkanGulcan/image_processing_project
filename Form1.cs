...
namespace ImageProcessingProject
{
    public partial class Form1 : Form
    {
            ...
            try
            {
...

                {
                    switch (treeView1.SelectedNode.Text)
                    {
                        ...
                        case string s when s.Contains("Kontrast"):
                            int contrastValue = (int)trkThreshold.Value;
                            Bitmap contrastResult = ImageProcessor.KontrastUygula((Bitmap)picInput1.Image, contrastValue);
                            picOutput.Image = contrastResult;
                            break;
                        ...
                        case "Görüntü Yaklaştırma/Uzaklaştırma":
                            if (picInput1.Image != null)
                            {
                                ...
                                if (secilenOran > 0)
                                {
                                    Bitmap kaynakBmp = new Bitmap(picInput1.Image);
                                    Bitmap islenmisResim = ImageProcessor.GoruntuOlcekle(kaynakBmp, secilenOran);

                                    if (islenmisResim != null)
                                    {
                                        picOutput.Image = islenmisResim;
                                    }
                                }
                                else
                                {
                                    MessageBox.Show("Lütfen 0'dan büyük bir ölçekleme oranı giriniz.");
                                }
                            }
                            else
                            {
                                MessageBox.Show("İşlem yapılacak bir resim bulunamadı. Lütfen önce resim yükleyin.");
                            }
                            break;
        ...
        private void görüntüYakınlaştırmaUzaklaştırmaToolStripMenuItem_Click(object sender, EventArgs e)
        {
            btnUygula_Click(sender, e);
        }
        ...
        private void görüntüYakınlaştırmaUzaklaştırmaToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            btnUygula_Click(sender, e);
        }
